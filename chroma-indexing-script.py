...
def process_documents(input_dir, output_dir="./chroma_db"):
    """Process all supported documents and create an index using ChromaDB"""
    # Setup models
    ...
    try:
        ...
        # Create the index with ChromaDB
        if documents:
            ...
            # Create vector store using the collection
            ...
            print(f"Index created and saved to ChromaDB at {output_dir}")
            
            # Return the index for querying if needed
            return index
        else:
...
def query_index(output_dir="./chroma_db", query_text=""):
...
if __name__ == "__main__":
    ...
    index = process_documents(input_directory, output_directory)
    
    # Example query
    if index:
        query = "What are the main topics in these documents?"
        response = query_index(output_directory, query)
        if response:
            print(f"\nQuery: {query}")
